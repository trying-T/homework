...
### SOS_light.ino — SOS 摩斯电码信号灯

使用 ESP32 板载 LED 发送 SOS 求救信号（摩斯电码：`... --- ...`）。

- `shortBlink()` — 短闪烁（200ms 亮 / 200ms 灭），表示摩斯电码的 `.`
- `longBlink()` — 长闪烁（600ms 亮 / 200ms 灭），表示摩斯电码的 `-`
- `loop()` — 循环发送 S（短闪×3）→ O（长闪×3）→ S（短闪×3），字母间隔 500ms，单词间隔 2000ms，并通过串口输出当前发送的字母

![实验照片](lab01/151800364a1ebe29924425f858f78478.jpg)
